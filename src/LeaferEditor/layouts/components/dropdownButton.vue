<template>
  <a-button class="btn-box" :class="{ active }" :size="size" :disabled="disabled" v-bind="buttonProps"
    @click="handleClick">
    <template #icon>
      <slot></slot>
      <a-dropdown v-if="$slots.content" :popup-visible="computedPopupVisible" :trigger="trigger" :position="position"
        :popup-container="popupContainer" :hide-on-select="hideOnSelect" @select="handleSelect"
        @popup-visible-change="handlePopupVisibleChange">
        <a-button class="more-btn absolute! w12px! h12px! bottom-0 right-0 bg-transparent! transition translate-0"
          :disabled="disabled">
          <template #icon>
            <slot name="icon" :popup-visible="computedPopupVisible">
              <div class="i-svg:dropdown-more" style="width: 12px; height: 12px;"></div>
            </slot>
          </template>
        </a-button>
        <template #content>
...
import { type ButtonProps, TriggerEvent } from '@arco-design/web-vue'
