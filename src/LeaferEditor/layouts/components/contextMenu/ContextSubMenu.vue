<template>
  <div class="mx-context-menu" :class="[options.customClass, globalTheme]" :style="{
    ...
    left: `${position.x}px`,
    top: `${position.y}px`,
  }" data-type="ContextSubMenu" @click="onSubMenuBodyClick" @contextmenu="(e) => e.preventDefault()">
    <!--Child menu items-->
    <div class="mx-context-menu-items" ref="menu" :style="{
      top: `${scrollValue}px`,
    }">
      <slot>
        <template v-for="(item, i) in items" :key="i">
          <!--Menu Item-->
          <ContextMenuItem :clickHandler="item.onClick" :disabled="item.disabled" :hidden="item.hidden"
            :icon="item.icon" :iconFontClass="item.iconFontClass" :svgIcon="item.svgIcon" :svgProps="item.svgProps"
            :label="item.label" :customRender="(item.customRender as Function)" :customClass="item.customClass"
            :checked="item.checked" :shortcut="item.shortcut" :clickClose="item.clickClose"
            :clickableWhenHasChildren="item.clickableWhenHasChildren" :preserveIconWidth="item.preserveIconWidth !== undefined
              ? item.preserveIconWidth
              : options.preserveIconWidth
              " :showRightArrow="item.children && item.children.length > 0"
            :hasChildren="item.children && item.children.length > 0" @sub-menu-open="item.onSubMenuOpen"
            @sub-menu-close="item.onSubMenuClose">
            <template v-if="item.children && item.children.length > 0" #submenu>
              <!--Sub menu-->
              <ContextSubMenu :items="item.children" :maxWidth="item.maxWidth" :minWidth="item.minWidth"
                :adjustPosition="item.adjustSubMenuPosition !== undefined
    ...
    <div class="mx-context-menu-scroll" ref="scroll">
      ...
      <div v-show="overflow && scrollValue < 0" class="mx-context-menu-updown mx-context-no-clickable up"
        @click="onScroll(false)">
        <ContextMenuIconRight />
      </div>
      <div v-show="overflow && scrollValue > -scrollHeight" class="mx-context-menu-updown mx-context-no-clickable down"
        @click="onScroll(true)">
        <ContextMenuIconRight />
      </div>
    </div>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    function setAndFocusCurrentMenu(index?: number) {
      ...
      if (index !== undefined)
        currentItem = menuItems[Math.max(0, Math.min(index, menuItems.length - 1))] || null
