import { type App, h, render, type Slot } from "vue";
    ...
    ContextSubMenuWrapperConstructor,
    {
      ...
      onClose: () => {
        ...
        if (vnode.el?.parentElement) {
          const menu = (
            vnode.el.parentElement as HTMLElement
          ).querySelector<HTMLElement>(".mx-context-menu");
          if (menu) {
...
function $contextmenu(
  ...
  return (component as unknown as Record<string, unknown>)
    .exposed as ContextMenuInstance;
}
